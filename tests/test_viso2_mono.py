...
import os
import numpy as np
from viso2 import VisualOdometryMono, Mono_parameters
...
class TestLibVisOMonoExecution(unittest.TestCase):
    ...
    def test_simple_trial_run(self):
        ...
        subject = VisualOdometryMono(params)
        ...
        for time in range(num_frames):
            frame = create_frame(time / num_frames)
            ...
            if time <= 0:
                ...
                # Otherwise, there should be _some_ motion registered.
                if inliers > 0:
                    got_inliers = True
                if matches > 0:
                    got_matches = True
